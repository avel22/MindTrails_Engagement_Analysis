...
# variables ----
#--------------------------------------------------------------------------------#
p_ids <- dat.3$participant %>% dplyr::select(participant_id) %>% filter(!(participant_id %in% c(776,169))) %>% pull()
...
#datafram to store all variables to be used for imputation
...
sink(here("Scripts2","Tables_Figures","Tables","cronbach.txt"))
cronbach_measures
sink()

sink(here("Scripts2","Tables_Figures","Tables","cronbach_rounded2.txt"))
cronbach_measures_rounded
sink()

gdata::write.fwf(cronbach_measures_rounded,file=here("Scripts2","Tables_Figures","Tables","cronbach_rounded2.txt"),sep="\t", quote=F, rownames=T)
...
sink(here("Scripts2","Tables_Figures","Tables","desc_stats_outcomes_scores_revised.txt"))
...
save(outcomes.scores.df,session_outcomes, file = here("Scripts2","Data2","Test699","outcomes_df_for_imputation.RData"))
