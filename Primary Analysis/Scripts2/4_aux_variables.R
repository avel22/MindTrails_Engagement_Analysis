...
# variables ----
#--------------------------------------------------------------------------------#
p_ids <- dat.3$participant %>% select(participant_id) %>% filter(!(participant_id %in% c(776,169))) %>% pull()
...
completion<- completion %>% filter(!(participant_id %in% c(776,169)))
...
completion <- merge(completion,
                    dat.3$participant[, c("participant_id", "exclude_analysis",
...
nrow(compl_itt_iv) == 697

write.csv(fct_res_itt_unrestricted,
          file = here("Scripts2","results","potential_cat_ord_aux_vars.csv"),
          row.names = FALSE)
...
sink_test_diffs <- function(itt_unrestricted_df,
  ...
  sink(file = paste0(here("Scripts2","results"),"/", outcome, "_", aux_var, "_diffs.txt"))
...
# ---------------------------------------------------------------------------- #
# Test for auxiliary demographic variables age ----
# ---------------------------------------------------------------------------- #
...
ggplot(compl_itt_iv, aes(x = age))+geom_histogram()
summary(compl_itt_iv$age)

cor.test(compl_itt_iv$miss_session_assess_prop, compl_itt_iv$age, method = c("pearson"))

plot(compl_itt_iv$miss_session_assess_prop, compl_itt_iv$age)
abline(lm(compl_itt_iv$miss_session_assess_prop~ compl_itt_iv$age))

GoodmanKruskalGamma(compl_itt_iv$miss_session_assess_prop,compl_itt_iv$age, conf.level = .95)
...
save(aux_vars, file = here("Scripts2","Data_Primary_Analysis","aux_vars.RData"))
