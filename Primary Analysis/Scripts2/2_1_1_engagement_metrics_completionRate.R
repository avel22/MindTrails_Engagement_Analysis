...
### Asked Jeremy Eberle during MindTrails breakout room meeting on 03/18/2022 if it was okay to use his script and he said yes
...
cbm_ids <- 
  unique(dat$angular_training[dat$angular_training$conditioning %in% cbm_conditions, 
                              "participant_id"])
#tidyverse way
#cbm_ids2 <- dat$angular_training %>% select(participant_id) %>% unique() %>% pull()
...
# output_cbm2 <- dat$angular_training %>% 
...
assessments.table2 <- createAssessmentTable(dat.3 = dat.3,columnsInterest = columns)
...
tasks_tl <- dat.3$task_log %>% select(session_only,task_name) %>% filter(!(task_name %in% c(1:5,"Covid19","SESSION_COMPLETE"))) %>%  unique() %>% group_by(session_only)


n_session_task_tl <- dat.3$task_log %>% select(session_only,tag, task_name) %>% filter(!(task_name %in% c(1:5,"Covid19","SESSION_COMPLETE", "recognitionRatings"))) %>%
...
save(final_completion,completion,check_completion2, file = here("Scripts2","Data2","completionRate.RData"))
