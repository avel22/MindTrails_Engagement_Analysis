...
mods_on_imps <- function(imputationList){
  for(i in 1:length(imputationList)){
    ...
    imputationList[[i]]$eng_label <- as.factor(imputationList[[i]]$eng_label)
    
    #relevel so that more time spent "1" is reference group
    imputationList[[i]] <- within(imputationList[[i]], eng_cluster <- relevel(eng_cluster, ref = 1))
    imputationList[[i]] <- within(imputationList[[i]], eng_label <- relevel(eng_label, ref = "More Time Spent"))
...
print(levels(mitml_list_oa_2[[1]]$eng_label))
print(levels(mitml_list_oa_2[[1]]$eng_cluster))

#set df as though data were complete
#model with 7 timepoints (OASIS outcome)
df_test7 <- lme(fixed = outcome ~ t1 + t2 + eng_cluster + t1 * eng_cluster + t2 * eng_cluster + cred_on_gmc ,
...
df_test4 <- lme(fixed = outcome ~ t1 + t2 + eng_cluster + t1 * eng_cluster + t2 * eng_cluster + cred_on_gmc ,
    random = ~ t1 + t2 | participant_id,
...
save.image(file = here("Data_Sensitivity_Analysis","7_1_mitml_pooled_results_MTS_Reference_Group.RData"))
