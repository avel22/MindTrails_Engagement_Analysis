...
compute_pred_means_4 <- function(modelList) {
  ...
  # engagement cluster More Time Spent (1) is reference group
  eng_cluster <- rep(c("More Time Spent", "Less Time Spent"), each = 7)
  assessment <- rep(c("Baseline", "Session 3", 
                      "Session 5", "Follow-Up"), times = 2)
  
  pred_means <- data.frame(eng_cluster, assessment)
  
  pred_means$eng_cluster_encoded <- ifelse(pred_means$eng_cluster == "More Time Spent", 0, 1)
  ...
  for (i in 1:nrow(pred_means)) {
    eng_cluster_encoded <- pred_means$eng_cluster_encoded[i]
    t1 <- pred_means$t1[i]
    t2 <- pred_means$t2[i]
    
    ctr <- paste0("(Intercept) + ",
                  "eng_labelLess Time Spent * ", eng_cluster_encoded, " + ",
                  "t1 * ", t1, " + ",
                  "t2 * ", t2, " + ",
                  "`t1:eng_labelLess Time Spent` * (", t1, "*", eng_cluster_encoded, ") + ",
                  "`t2:eng_labelLess Time Spent` * (", t2, "*", eng_cluster_encoded, ") +",
                  "cred_on_gmc * ",0)
...
create_plot <- function(pred_means, hidden_pts, title, y_title, scale_min, 
  ...
  ggplot(pred_means, 
         aes(x = assessment, y = mean, 
             group = eng_cluster, color = eng_cluster, linetype = eng_cluster)) +
    geom_line() +
...
pred_means_OA <- 
  compute_pred_means(modelList_oa_lme)


pred_means_dass21 <- 
  compute_pred_means(modelList_dass21_lme)
...
pred_means_rr_pos <-  
  compute_pred_means(modelList_rr_pos_lme)

pred_means_bbsiq <- 
  compute_pred_means(modelList_bbsiq_lme)
...
png(here("Figures","8_Outcomes_Predicted_Means","all_est_means_outcomes_MTS_Reference_Group.png"), units="in", width=10, height=8, res=300)
est_means_outcomes
dev.off()

png(here("Figures","8_Outcomes_Predicted_Means","oa_means_outcomes_MTS_Reference_Group.png"), units="in", width=10, height=8, res=300)
p_OA
dev.off()

png(here("Figures","8_Outcomes_Predicted_Means","dass21_means_outcomes_MTS_Reference_Group.png"), units="in", width=10, height=8, res=300)
p_dass21
dev.off()

png(here("Figures","8_Outcomes_Predicted_Means","rr_neg_means_outcomes_MTS_Reference_Group.png"), units="in", width=10, height=8, res=300)
p_rr_neg
dev.off()

png(here("Figures","8_Outcomes_Predicted_Means","rr_pos_means_outcomes_MTS_Reference_Group.png"), units="in", width=10, height=8, res=300)
p_rr_pos
dev.off()

png(here("Figures","8_Outcomes_Predicted_Means","bbsiq_means_outcomes_MTS_Reference_Group.png"), units="in", width=10, height=8, res=300)
p_bbsiq
dev.off()
